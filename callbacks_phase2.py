    ...
    def scheduler(epoch, lr):
        if epoch < hyperparameters.LEARNING_RATE_DECAY_STRATPOINT:
            if epoch % hyperparameters.LEARNING_RATE_DECAY_STEP == 0:
                lr = lr * float(tf.math.exp(hyperparameters.LEARNING_RATE_DECAY_PARAMETERS))
        # Ensure learning rate is a float and doesn't fall below a threshold
        lr = max(float(lr), 1e-6)
        return float(lr)  # Ensure that the return value is always a float
...
#     def on_train_end(self, logs=None):
#         self.model.set_weights(self.model_best_weights)
#         print(f"\nBest weights are set, Best Epoch was : {self.best_epoch+1}\n")
...
# def LearningRateScheduler():
#     def scheduler(epoch, lr):
#         # Parameters for Warm-Up and Cosine Annealing
#         warmup_epochs = hyperparameters.WARMUP_EPOCHS  # Number of epochs to warm up
#         max_lr = hyperparameters.MAX_LEARNING_RATE     # Maximum learning rate during warm-up
#         min_lr = hyperparameters.MIN_LEARNING_RATE     # Minimum learning rate (end of annealing)
#         total_epochs = hyperparameters.TOTAL_EPOCHS    # Total training epochs

#         # Warm-Up Phase
#         if epoch < warmup_epochs:
#             lr = (max_lr / warmup_epochs) * (epoch + 1)
